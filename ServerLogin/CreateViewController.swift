...
class CreateViewController: UIViewController, UITextFieldDelegate {
        ...
        let task = session.dataTask(with: request) { (responseData, response, responseError) in
            ...
            if let utf8Data = String(data: receivedData, encoding: .utf8) {
                DispatchQueue.main.async { // for Main Thread Checker
                    let alert = UIAlertController(title: "\(utf8Data)",
                                                           message: "Sign in", preferredStyle: .alert)
    ...
    @IBAction func buttonSave() {
        // 필요한 세 가지 자료가 모두 입력 되었는지 확인 if textID.text == "" {
        if textID.text == "" {
            //labelStatus.text = "ID를 입력하세요";
            let alert = UIAlertController(title: "ID를 입력하세요",
                                                                                               message: "Sign in Failed!!", preferredStyle: .alert)
                                                                 alert.addAction(UIAlertAction(title: "OK", style: .default, handler: { action in
                                                                     alert.dismiss(animated: true, completion: nil)}))
                                                        self.present(alert, animated: true)
            return;
        ...
        if textPassword.text == "" {
            ...
            let alert = UIAlertController(title: "Password를 입력하세요",
                                                   message: "Sign in Failed!!", preferredStyle: .alert)
                     alert.addAction(UIAlertAction(title: "OK", style: .default, handler: { action in
                         alert.dismiss(animated: true, completion: nil)}))
            self.present(alert, animated: true)
        ...
        if textName.text == "" {
            ...
            let alert = UIAlertController(title: "사용자 이름을 입력하세요",
                                                   message: "Sign in Failed!!", preferredStyle: .alert)
                     alert.addAction(UIAlertAction(title: "OK", style: .default, handler: { action in
                         alert.dismiss(animated: true, completion: nil)}))
            self.present(alert, animated: true)
        ...
        let urlString: String = "http://condi.swu.ac.kr/student/T09/login/insertUser.php"
